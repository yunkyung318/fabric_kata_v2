...
function register_org_admins() {
  ...
  register_org_admin org orgadmin orgadminpw
...
function enroll_org_admins() {
  ...
  enroll_org_admin orderer  org orgadmin orgadminpw
...
function create_channel_MSP() {
  ...
  create_channel_org_MSP org orderer $ORG_NS

  extract_orderer_tls_cert org orderer
...
function create_channel_org_MSP() {
  ...
  ORG_MSP_DIR=${TEMP_DIR}/channel-msp/Organizations/${org}/msp
...
function extract_orderer_tls_cert() {
  ...
  ORDERER_TLS_DIR=${TEMP_DIR}/channel-msp/Organizations/${org}/orderers/${org}-${orderer}/tls
  mkdir -p $ORDERER_TLS_DIR/signcerts

  kubectl get secret -n $NS ${org}-${orderer}-tls-cert -o json \
    | jq -r '.data["tls.crt"]' \
    | base64 -d \
...
function create_genesis_block() {
  ...
  cat ${PWD}/config/org/configtx-template.yaml | envsubst > ${TEMP_DIR}/configtx.yaml
...
function join_channel_orderers() {
  ...
  join_channel_orderer org orderer
...
function join_channel_orderer() {
  ...
  osnadmin channel join \
    ...
    --ca-file         ${TEMP_DIR}/channel-msp/Organizations/${org}/orderers/${org}-${orderer}/tls/signcerts/tls-cert.pem \
...
function join_channel_peer() {
  ...
  peer channel join \
    ...
    --orderer     org-orderer.${DOMAIN} \
    --connTimeout ${ORDERER_TIMEOUT} \
    --tls         \
    --cafile      ${TEMP_DIR}/channel-msp/Organizations/org/orderers/org-orderer/tls/signcerts/tls-cert.pem 
}
