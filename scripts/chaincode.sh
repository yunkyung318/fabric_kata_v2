...
function invoke_chaincode() {
  ...
  peer chaincode invoke \
    ...
    -C              $CHANNEL_NAME \
    -c              $@ \
    --orderer       org-orderer.${DOMAIN}:${NGINX_HTTPS_PORT} \
    --connTimeout   ${ORDERER_TIMEOUT} \
    --tls --cafile  ${TEMP_DIR}/channel-msp/Organizations/org/orderers/org-orderer/tls/signcerts/tls-cert.pem \
    ${INVOKE_EXTRA_ARGS}
...
function approve_chaincode() {
  ...
  echo "echo $CORE_PEER_MSPCONFIGPATH"
  echo $CORE_PEER_MSPCONFIGPATH
  
  echo "ls -l $CORE_PEER_MSPCONFIGPATH"
  ls -l $CORE_PEER_MSPCONFIGPATH

  echo "echo $CORE_PEER_TLS_ROOTCERT_FILE"
  echo $CORE_PEER_TLS_ROOTCERT_FILE

  echo "cat $CORE_PEER_TLS_ROOTCERT_FILE"
  cat $CORE_PEER_TLS_ROOTCERT_FILE
  
  echo "echo $FABRIC_CFG_PATH"
  ...
  peer lifecycle \
    ...
    --package-id    ${cc_id} \
    --sequence      1 \
    --orderer       org-orderer.test-network.svc.cluster.local:6050
      #.${DOMAIN}:${NGINX_HTTPS_PORT} \
    --connTimeout   ${ORDERER_TIMEOUT} \
    --tls --cafile  ${TEMP_DIR}/channel-msp/Organizations/org/orderers/org-orderer/tls/signcerts/tls-cert.pem \
    ${APPROVE_EXTRA_ARGS}
