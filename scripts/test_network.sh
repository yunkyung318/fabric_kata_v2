...
function launch_orderers() {
  ...
  apply_template kube/org/org-orderer.yaml $ORG_NS

  kubectl -n $ORG_NS rollout status deploy/org-orderer
...
function launch_peers() {
  ...
  apply_template kube/org/org-peer1.yaml $ORG_NS
  apply_template kube/org/org-peer2.yaml $ORG_NS

  kubectl -n $ORG_NS rollout status deploy/org-peer1
  kubectl -n $ORG_NS rollout status deploy/org-peer2
...
function create_orderer_local_MSP() {
  ...
  create_node_local_MSP orderer $org $orderer $csr_hosts $ORG_NS
}
...
function create_local_MSP() {
  ...
  create_orderer_local_MSP org orderer

  create_peer_local_MSP org peer1 $ORG_NS 
  create_peer_local_MSP org peer2 $ORG_NS
